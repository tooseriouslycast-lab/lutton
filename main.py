"""
SwingBot: Claw to the Moon  v5.3 - MOBILE WEB EDITION (Pygbag-compatible rebuild)
"""
import pygame
import asyncio
import math
...
# Fonts
f_big = pygame.font.SysFont("Courier New", 58, bold=True)
f_med = pygame.font.SysFont("Courier New", 30, bold=True)
f_sm = pygame.font.SysFont("Courier New", 18)
f_xs = pygame.font.SysFont("Courier New", 14)
...
UPGRADE_DEFS = [
    ...
    dict(key='thruster', name='Thruster', emoji='🚀', desc='Hold bottom center = rocket boost',
    ...
    def draw(self, surf, tick, cam_x=0):
        ...
        for i,d in enumerate(UPGRADE_DEFS):
            ...
            if lvl < maxl:
                ...
                txt("MAX", f_med, C_GOLD, PW-62, ry+22)
...
class Robot:
    ...
    def draw(self, surf, cam_y, cam_x, tokens, store):
        # Draw rope/claw
        sx = int(self.wx - cam_x); sy = int(self.wy - cam_y)
        cx = int(self.claw_wx - cam_x); cy = int(self.claw_wy - cam_y)
        pygame.draw.line(surf, (180,180,200), (sx,sy), (cx,cy), 3)
        pygame.draw.circle(surf, (50,60,80), (cx,cy), 8)
        pygame.draw.circle(surf, (200,220,255), (cx-3,cy-3), 3)
        for i,(tx,ty) in enumerate(self.trail):
            a = i/len(self.trail) if self.trail else 0
            r = max(1, int(6*(1-a)))
            pygame.draw.circle(surf, (120,140,180), (int(tx-cam_x), int(ty-cam_y)), r)
        body = pygame.Rect(0,0,self.BW,self.BH)
        body.center = (sx, sy)
        pygame.draw.rect(surf, C_CHASSIS, body, border_radius=6)
        pygame.draw.rect(surf, C_CHASSIS_D, body, 2, border_radius=6)
        eye_x = sx; eye_y = sy - 6
        pygame.draw.circle(surf, C_EYE_A if self.eye_blink==0 else C_EYE_B, (eye_x, eye_y), 6)
        pygame.draw.circle(surf, (20,20,30), (eye_x, eye_y), 2)
        if self.thrusting:
            for i in range(4):
                px = sx + random.randint(-8,8)
                py = sy + self.BH//2 + random.randint(2,8)
                pygame.draw.circle(surf, C_THRUST, (px,py), random.randint(2,4))

# Background and UI
def draw_background(surf, cam_y, tick):
    surf.fill((6,8,20))
    for i in range(60):
        x = (i*37) % (W+120)
        y = (i*53 + (tick//5)) % (H+200) - 100
        pygame.draw.circle(surf, (80,90,140), (x, int(y - cam_y*0.002 % (H+200))), 1)
    plat_top = GROUND_Y - PLATFORM_H
    pygame.draw.rect(surf, (30,34,48), (PLATFORM_X-PLATFORM_W//2, plat_top-cam_y, PLATFORM_W, PLATFORM_H))
    pygame.draw.rect(surf, (60,70,90), (0, H-4, W, 4))

def draw_particles(surf, cam_y, cam_x):
    for p in particles[:]:
        p.update(); p.draw(surf, cam_y, cam_x)
        if p.life <= 0: particles.remove(p)
    for p in coin_particles[:]:
        p.update(); p.draw(surf, cam_y, cam_x)
        if p.life <= 0: coin_particles.remove(p)

def draw_tokens(surf, tokens, cam_y, cam_x):
    for t in tokens:
        t.draw(surf, cam_y, cam_x)

def draw_hud(surf, store, stats):
    s = f_med.render(f"Coins: {store.coins}", True, C_GOLD)
    surf.blit(s, (12,10))
    t = f_xs.render(f"Tokens: {stats.get('tokens',0)}", True, C_WHITE)
    surf.blit(t, (12, 48))
...
def draw_start_screen(surf):
    ...
    title = f_big.render("SwingBot", True, C_GOLD)
    sub = f_med.render("Tap to start — Left=Pump Right=Claw Bottom=Thrust", True, C_WHITE)
    surf.blit(title, (W//2 - title.get_width()//2, H//2-90))
    surf.blit(sub, (W//2 - sub.get_width()//2, H//2))

def draw_dead_screen(surf, stats):
    overlay = pygame.Surface((W,H), pygame.SRCALPHA)
    overlay.fill((0,0,0,180))
    surf.blit(overlay, (0,0))
    title = f_med.render("You Crashed! Tap to restart", True, C_DANGER)
    surf.blit(title, (W//2 - title.get_width()//2, H//2-20))
...
async def main():
    global touch_claw, touch_pump_left, touch_pump_right, touch_thrust
    ...
    while True:
        ...
        for ev in pygame.event.get():
            ...
            if ev.type in (pygame.MOUSEBUTTONUP, pygame.FINGERUP):
                touch_claw = False; touch_thrust = False
        ...
        if state == "play" and not paused:
            ...
            cam_y += ((robot.wy - H*0.72) - cam_y) * 0.08
            cam_x += (robot.wx*0 - cam_x) * 0.08
        draw_surface = pygame.Surface((W, H))
        draw_background(draw_surface, cam_y, tick)
        draw_tokens(draw_surface, tokens, cam_y, cam_x)
        robot.draw(draw_surface, cam_y, cam_x, tokens, store)
        draw_particles(draw_surface, cam_y, cam_x)
        store.draw(draw_surface, tick, cam_x)
        draw_hud(draw_surface, store, stats)
        if state == "start": draw_start_screen(draw_surface)
        if state == "play" and not robot.alive:
            draw_dead_screen(draw_surface, stats)
            state = "start"
            tokens, robot, cam_y, cam_x = new_game(store)
